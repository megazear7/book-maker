...
import { generateAudio } from "./services/generate-audio.js";
...
while (index < 1 && nextChapter) {
    ...
    const audioBuffer = await generateAudio(client, writtenParts.join('\n'));
    await fs.writeFile(`data/${config.book}/chapters/chapter-${nextChapter.index}/chapter-${nextChapter.index}.wav`, audioBuffer);
