...
export async function getBookAudio(bookId: BookId): Promise<Readable> {
    ...
    for (const chapter of book.chapters) {
        for (let i = 0; i < chapter.parts.length; i++) {
            ...
            const audioId = await getChapterPartAudioId(book.id, chapter.number, partNumber);
...
async function combineMP3Files(inputFiles: string[], outputFile: string) {
    return new Promise((resolve, reject) => {
        ...
        .on('end', () => resolve('MP3 files combined successfully'))
